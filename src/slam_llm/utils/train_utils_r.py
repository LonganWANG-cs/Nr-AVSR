...
class CustomCrossEntropyLoss(nn.Module):
    ...
    # def forward(self, logits, target,forget_rate):
    #     # Flatten logits and targets for easy processing
    ...
    #     # Calculate log softmax of logits
    ...
    #     final_loss = loss[ind_update].mean()
    #     return final_loss
    def forward(self, logits, target,forget_rate):
        # Flatten logits and targets for easy processing
        logits = logits.view(-1, logits.size(-1))
...
def train_r(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    """
    ...
    best_val_acc = 0.0
    forget_rate=0.5
    criterion = CustomCrossEntropyLoss(ignore_index=tokenizer.pad_token_id)
    rate_schedule = np.ones(train_config.num_epochs) * forget_rate
    rate_schedule[0:train_config.num_epochs] = np.linspace(0, forget_rate **1, train_config.num_epochs)
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # loss = outputs.loss
                loss = criterion(outputs.logits, batch["labels"],rate_schedule[epoch])
